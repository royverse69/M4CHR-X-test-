...
import { rand, choose, TAU, alignYAxis } from './util.js';
import { buildHumanoid } from './enemies.js';

// the maps everyone sees, and the ones still being tested (unlocked with a code on the main menu)
export const LEVELS = [{ key: 'district', name: 'DOODLE DISTRICT', blurb: 'streets, rooftops and fire escapes' }];
export const DEV_LEVELS = [{ key: 'jungle', name: 'DOODLE JUNGLE', blurb: 'canopies, vines and a lost temple · test map' }, { key: 'mexico', name: 'DOODLE MEXICO', blurb: 'a sun-baked plaza · piñatas, tacos and mariachi · test map' }];
export const ALL_LEVELS = [...LEVELS, ...DEV_LEVELS];

function createBuilder(scene, world) {
  const geos = {}; const L = { rings: [], spawns: [], snipers: [], pickups: [], animated: [], meshes: [], playerStart: new THREE.Vector3(0, 0, 42), bounds: { minX: -55, maxX: 55, minZ: -55, maxZ: 55 }, arenaSpawns: [], grappleMovers: [], breakables: [], props: [], style: null, key: 'district' };
  // solid (filled) ink gets its own merged mesh per colour
  const addGeo = (g, ink, fill = false) => { const k = ink + (fill ? 'f' : ''); (geos[k] || (geos[k] = [])).push(g); };
  const mesh = (geo, ink, fill = false) => new THREE.Mesh(geo, makeInkMaterial({ ink, fill, side: fill ? THREE.DoubleSide : THREE.FrontSide }));
  ...
  function box(x, y, z, w, h, d, o = {}) {
    const g = new THREE.BoxGeometry(w, h, d); g.translate(x, y + h / 2, z); addGeo(g, o.ink ?? INK.BLUE, !!o.fill);
  ...
  function finish() {
    for (const key in geos) {
      const fill = key.endsWith('f'); const merged = mergeGeometries(geos[key], false);
      const m = new THREE.Mesh(merged, makeInkMaterial({ ink: parseInt(key, 10), fill, side: fill ? THREE.DoubleSide : THREE.FrontSide }));
      m.matrixAutoUpdate = false; scene.add(m); L.meshes.push(m);
    }
    world.finalize();
    return L;
  }
  ...
  // a loose prop: its own group of meshes (built around the origin) plus the numbers the prop
  // physics needs; it becomes a collider once the game wraps it
  function prop(kind, x, y, z, half, build, o = {}) {
    const g = new THREE.Group(); build(g); g.position.set(x, y + half.y, z); g.rotation.y = o.yaw || 0; scene.add(g); L.meshes.push(g);
    L.props.push({ kind, group: g, half: { ...half }, mass: o.mass ?? 1, snap: o.snap || 'cube', ink: o.ink ?? INK.BLUE, radius: o.radius, footHalf: o.footHalf, start: { x, y, z, yaw: o.yaw || 0 } });
  }
  // birds that circle the map and can be hooked, like the paper planes
  function birds(n, baseR, baseH, o = {}) {
    const sc = o.scale || 1;
    for (let i = 0; i < n; i++) {
      const g = new THREE.Group(); const ink = o.ink ?? INK.BLACK;
      g.add(mesh(new THREE.SphereGeometry(0.5 * sc, 8, 6).scale(1, 0.8, 1.7), ink), mesh(new THREE.ConeGeometry(0.2 * sc, 0.9 * sc, 5).rotateX(Math.PI / 2).translate(0, 0.05 * sc, 1.2 * sc), INK.ORANGE));
      g.add(mesh(new THREE.BoxGeometry(0.5 * sc, 0.5 * sc, 1.0 * sc).translate(0, 0.25 * sc, -1.1 * sc), i % 2 ? INK.PINK : INK.GREEN));
      const wl = mesh(new THREE.BoxGeometry(2.3 * sc, 0.08 * sc, 0.9 * sc).translate(1.15 * sc, 0, 0), ink), wr = mesh(new THREE.BoxGeometry(2.3 * sc, 0.08 * sc, 0.9 * sc).translate(-1.15 * sc, 0, 0), ink);
      wl.position.x = 0.3 * sc; wr.position.x = -0.3 * sc; g.add(wl, wr);
      scene.add(g); L.meshes.push(g); L.grappleMovers.push({ mesh: g, radius: 1.9 * sc });
      const r = baseR + i * (o.rStep ?? 10), h = baseH + i * (o.hStep ?? 5), ph = i * 1.9, sp = (o.speed ?? 0.1) + i * 0.012, dir = i % 2 ? -1 : 1;
      L.animated.push({ mesh: g, update: (t) => { const a = dir * t * sp + ph, a2 = a + dir * 0.05; g.position.set(Math.cos(a) * r, h + Math.sin(a * 1.7) * 2.5, Math.sin(a) * r * 0.8); g.lookAt(Math.cos(a2) * r, h + Math.sin(a2 * 1.7) * 2.5, Math.sin(a2) * r * 0.8); const flap = Math.sin(t * 7 + ph) * 0.55; wl.rotation.z = flap; wr.rotation.z = -flap; } });
    }
  }
  return { L, addGeo, collider, box, slab, wallX, wallZ, stairs, rail, cyl, sphere, ring, spawn, sniper, pickup, finish, planes, birds, prop, mesh, scene, world };
}
...
// ============================ map 3: Doodle Jungle ============================
// Drawn in felt tip on graph paper. A lost temple in a clearing, giant trees whose canopies are
// platforms joined by rope bridges, vines to swing from (some of them swinging themselves), a
// river cut into the floor, a waterfall you can walk behind, and an expedition camp. Crates,
// barrels, boulders, coconuts and idols are loose: shoot them, slash them or blow them across
// the clearing and they tumble, bounce and settle wherever they land.
function buildJungle(B, arena = false) {
  const { L, box, stairs, rail, cyl, sphere, ring, spawn, sniper, pickup, addGeo, collider, scene, prop, birds, mesh } = B;
  const OR = INK.ORANGE, GR = INK.GREEN, PK = INK.PINK, BK = INK.BLACK, BL = INK.BLUE;
  L.key = 'jungle'; L.playerStart.set(0, 0, 44); const P = 80, T = 8, PH = 22, E = P + T / 2; L.bounds = { minX: -P, maxX: P, minZ: -P, maxZ: P };
  // felt tip on a field notebook: warmer paper, a green grid, sepia where the district has graphite
  L.style = { paper: [0.955, 0.948, 0.865], lines: 1, inks: [[0.06, 0.30, 0.52], [0.86, 0.12, 0.20], [0.34, 0.21, 0.10], [0.95, 0.55, 0.10], [0.10, 0.55, 0.22], [0.90, 0.32, 0.55]] };
  const NG = { noNav: true, noGrapple: true };

  // ---------------- the floor, with a river cut half a metre into it ----------------
  const RIVER = [[-50, 62], [-38, 56], [-24, 50], [-10, 46], [6, 44], [22, 40], [38, 32], [52, 22], [64, 10], [80, -6]];
  const riverX = (z) => { for (let i = 0; i < RIVER.length - 1; i++) { const [z1, x1] = RIVER[i], [z2, x2] = RIVER[i + 1]; if (z <= z2) { const t = Math.max(0, (z - z1) / (z2 - z1)); const e = t * t * (3 - 2 * t); return x1 + (x2 - x1) * e; } } return RIVER[RIVER.length - 1][1]; };
  const riverW = (z) => (z < -44 ? 13 : 7 + 2 * Math.sin(z * 0.21));
  box(0, -2, 0, 2 * E, 1.5, 2 * E, { ink: GR });                           // the river bed, everywhere
  box(0, -0.5, (-E - 50) / 2, 2 * E, 0.5, 50 - (-E) , { ink: GR });          // solid ground north of the source
  for (let z0 = -50; z0 < E; z0 += 4) {
    const zc = z0 + 2, x = riverX(zc), w = riverW(zc), xw = x - w / 2, xe = x + w / 2;
    box((-E + xw) / 2, -0.5, zc, xw + E, 0.5, 4, { ink: GR }); box((xe + E) / 2, -0.5, zc, E - xe, 0.5, 4, { ink: GR });
    box(x, -0.32, zc, w + 0.5, 0.08, 4.02, { noCollide: true, ink: BL, fill: true });
  }
  box(0, -2.3, 0, 700, 1, 700, { noCollide: true, ink: GR });                // the world beyond the walls
  // stepping stones and a log across the water
  for (const [x, z] of [[27.6, 40], [30.4, 41.5], [33.4, 40.2], [44.2, 4], [47.4, 5.4]]) box(x, -0.5, z, 1.3, 0.62, 1.3, { ink: BL });

  // ---------------- the hedge walls; unseen, unhookable above, and a lid ----------------
  box(0, 0, -P, 2 * E, PH, T, { ink: GR }); box(0, 0, P, 2 * E, PH, T, { ink: GR }); box(-P, 0, 0, T, PH, 2 * E, { ink: GR }); box(P, 0, 0, T, PH, 2 * E, { ink: GR });
  collider(0, PH, -P, 2 * E, 44, T, NG); collider(0, PH, P, 2 * E, 44, T, NG); collider(-P, PH, 0, T, 44, 2 * E, NG); collider(P, PH, 0, T, 44, 2 * E, NG);
  collider(0, 66, 0, 2 * E + 60, 8, 2 * E + 60, NG);
  for (let i = -7; i <= 7; i++) { const t = i * 11; for (const [x, z] of [[t, -P], [t, P], [-P, t], [P, t]]) { const r = 3 + ((i + 7) % 3); const g = new THREE.SphereGeometry(r, 9, 7); g.scale(1, 0.7, 1); g.translate(x + ((i * 7) % 5) - 2, PH + r * 0.3, z + ((i * 3) % 5) - 2); addGeo(g, GR); } }

  // ---------------- trees: a trunk, roots, branches to hop along, a canopy you can stand on ----------------
  const canopyTop = (cy) => cy + 0.2;
  const tree = (x, z, cy, cr, tr, o = {}) => {
    cyl(x, 0, z, tr, cy - cr * 0.15, { seg: 9, ink: BK });
    for (let k = 0; k < 3; k++) { const a = k * TAU / 3 + (x + z) * 0.13; box(x + Math.cos(a) * (tr + 0.5), 0, z + Math.sin(a) * (tr + 0.5), 1.3, 0.9 + (k % 2) * 0.4, 1.3, { ink: BK }); }
    const n = 5 + Math.floor(cr / 2.5);
    // flattened leaf blobs, sunk so their tops are only knee to waist high on the platform: cover, not a blindfold
    for (let k = 0; k < n; k++) { const a = k * TAU / n + 0.4 + x * 0.01, rr = k === 0 ? 0 : cr * 0.55, r = cr * (k === 0 ? 0.72 : 0.42 + ((k * 7) % 4) * 0.05); const g = new THREE.SphereGeometry(r, 10, 7); g.scale(1, 0.42, 1); g.translate(x + Math.cos(a) * rr, cy - 1.1 + ((k * 3) % 3) * 0.2, z + Math.sin(a) * rr); addGeo(g, GR); }
    box(x, cy - 0.3, z, cr * 1.36, 0.5, cr * 1.36, { ink: GR }); ring(x, cy + 2.6, z, 'y');
    for (const [dir, h, len] of o.branches || []) {
      const dx = dir === '+x' ? 1 : dir === '-x' ? -1 : 0, dz = dir === '+z' ? 1 : dir === '-z' ? -1 : 0; const cx = x + dx * (tr + len / 2 - 0.3), cz = z + dz * (tr + len / 2 - 0.3);
      box(cx, h, cz, dx ? len : 0.75, 0.7, dz ? len : 0.75, { ink: BK });
      const g = new THREE.SphereGeometry(len * 0.32, 8, 6); g.scale(1, 0.55, 1); g.translate(x + dx * (tr + len - 0.3), h + 1.0, z + dz * (tr + len - 0.3)); addGeo(g, GR);
      ring(x + dx * (tr + len + 1.2), h + 0.9, z + dz * (tr + len + 1.2), 'y');
    }
    for (const [a, len] of o.vines || []) { const vx = x + Math.cos(a) * cr * 0.85, vz = z + Math.sin(a) * cr * 0.85; box(vx, cy - len, vz, 0.14, len, 0.14, { noCollide: true, ink: GR }); ring(vx, cy - len - 0.7, vz, 'y'); }
    for (let k = 0; k < (o.fruit || 0); k++) { const a = k * 2.4 + z * 0.02; sphere(x + Math.cos(a) * cr * 0.7, cy - 1.4 - (k % 2) * 0.7, z + Math.sin(a) * cr * 0.7, 0.32, { seg: 7, ink: k % 3 ? OR : PK }); }
  };
  tree(-46, -46, 18, 7, 1.6, { branches: [['+x', 7, 7], ['-z', 11, 6]], vines: [[0.8, 9], [3.6, 11]], fruit: 3 });
  tree(-14, -46, 18, 6, 1.4, { branches: [['+x', 6, 6]], vines: [[2.2, 8]], fruit: 2 });
  tree(18, -30, 22, 7.5, 1.8, { branches: [['-x', 9, 8], ['+z', 14, 7]], vines: [[0.5, 10], [4.2, 12]], fruit: 4 });
  tree(18, 26, 22, 6.5, 1.6, { branches: [['+x', 8, 7], ['-x', 13, 6]], vines: [[1.6, 9]], fruit: 3 });
  tree(-28, 54, 20, 7, 1.6, { branches: [['-x', 8, 7], ['-z', 12, 7]], vines: [[5.2, 9], [2.6, 12]], fruit: 3 });
  tree(32, 54, 20, 6, 1.4, { branches: [['+x', 7, 6], ['-z', 11, 6]], vines: [[1.2, 10]], fruit: 2 });
  tree(-30, -14, 28, 8, 2.0, { branches: [['+x', 10, 9], ['-z', 17, 8], ['+z', 22, 7]], vines: [[0.2, 12], [2.4, 14], [4.4, 10]], fruit: 5 });
  tree(48, -44, 25, 7.5, 1.8, { branches: [['-x', 9, 8], ['+z', 16, 7]], vines: [[2.9, 12], [5.5, 9]], fruit: 3 });
  tree(-56, 12, 15, 6, 1.4, { branches: [['+x', 6, 7], ['-z', 10, 6]], vines: [[0.4, 8]], fruit: 2 });
  tree(56, 8, 16, 6.5, 1.5, { branches: [['-x', 7, 7], ['+z', 11, 6]], vines: [[3.3, 9]], fruit: 3 });
  tree(-62, -20, 21, 6.5, 1.6, { branches: [['+x', 8, 8], ['+z', 14, 7]], vines: [[0.0, 11]], fruit: 2 });
  tree(62, 44, 26, 7, 1.8, { branches: [['-x', 9, 9], ['-z', 16, 7], ['-x', 21, 6]], vines: [[3.0, 12], [4.6, 10]], fruit: 4 });
  tree(0, 66, 14, 6, 1.4, { branches: [['-x', 6, 7], ['+x', 9, 6]], vines: [[1.0, 8]], fruit: 2 });
  tree(-8, 26, 11, 5, 1.2, { branches: [['-x', 5, 6]], vines: [[3.8, 7]], fruit: 2 });
  tree(40, -10, 13, 5.5, 1.3, { branches: [['-x', 6, 6], ['+z', 9, 6]], vines: [[2.0, 8]], fruit: 2 });

  // ---------------- rope bridges between canopies, high over everything ----------------
  const bridge = (x1, z1, x2, z2, y) => {
    const ax = Math.abs(x2 - x1) > Math.abs(z2 - z1); const len = ax ? Math.abs(x2 - x1) : Math.abs(z2 - z1); const cx = (x1 + x2) / 2, cz = (z1 + z2) / 2;
    box(cx, y - 0.25, cz, ax ? len : 2.4, 0.25, ax ? 2.4 : len, { ink: OR });
    for (let i = 0; i <= Math.floor(len); i += 1) { const t = i / len; box(x1 + (x2 - x1) * t, y, z1 + (z2 - z1) * t, ax ? 0.08 : 2.2, 0.03, ax ? 2.2 : 0.08, { noCollide: true, ink: BK }); }
    if (ax) { rail(x1, cz - 1.2, x2, cz - 1.2, y, { ink: BK }); rail(x1, cz + 1.2, x2, cz + 1.2, y, { ink: BK }); } else { rail(cx - 1.2, z1, cx - 1.2, z2, y, { ink: BK }); rail(cx + 1.2, z1, cx + 1.2, z2, y, { ink: BK }); }
    ring(cx, y - 1.4, cz, 'y');
  };
  bridge(-41.5, -46, -17.8, -46, canopyTop(18));
  bridge(18, -24.9, 18, 21.6, canopyTop(22));
  bridge(-23.2, 54, 27.9, 54, canopyTop(20));
  // a treehouse on the giant, a lookout on the tall one by the falls
  box(-30, 28.2, -14, 7.5, 0.4, 7.5, { ink: OR }); rail(-33.75, -17.75, -26.25, -17.75, 28.6, { ink: BK }); rail(-33.75, -10.25, -26.25, -10.25, 28.6, { ink: BK }); rail(-33.75, -17.75, -33.75, -10.25, 28.6, { ink: BK });
  box(-28.6, 28.6, -14, 3.6, 2.8, 3.6, { ink: OR }); box(-28.6, 31.4, -14, 4.4, 0.3, 4.4, { ink: GR }); box(-28.6, 28.6, -12.15, 1.2, 2.2, 0.1, { noCollide: true, ink: BK }); ring(-30, 33, -14, 'y');
  box(48, 25.2, -44, 5, 0.4, 5, { ink: OR }); rail(45.5, -46.5, 50.5, -46.5, 25.6, { ink: BK }); rail(50.5, -46.5, 50.5, -41.5, 25.6, { ink: BK }); rail(45.5, -41.5, 50.5, -41.5, 25.6, { ink: BK });

  // ---------------- vines that swing: hook the knot and ride it ----------------
  const swingVine = (x, top, z, len, phase, dirA) => {
    const knot = mesh(new THREE.SphereGeometry(0.42, 8, 6), GR); const line = mesh(new THREE.CylinderGeometry(1, 1, 1, 5), GR); scene.add(knot, line); L.meshes.push(knot, line);
    for (let k = 0; k < 3; k++) { const leaf = mesh(new THREE.BoxGeometry(0.9, 0.05, 0.4).translate(0.45, 0, 0), GR); leaf.rotation.set(0, k * 2.1, -0.4); knot.add(leaf); }
    L.grappleMovers.push({ mesh: knot, radius: 1.3 }); const from = new THREE.Vector3(x, top, z), dx = Math.cos(dirA), dz = Math.sin(dirA);
    L.animated.push({ mesh: knot, update: (t) => { const a = 0.45 * Math.sin(t * 1.1 + phase); knot.position.set(x + Math.sin(a) * len * dx, top - Math.cos(a) * len, z + Math.sin(a) * len * dz); knot.rotation.y = t * 0.6; alignYAxis(line, from, knot.position, 0.07); } });
  };
  swingVine(18, 21.7, 0, 12, 0, 0.3); swingVine(0, 19.7, 54, 11, 1.5, 1.4); swingVine(-30, 17.7, -46, 9, 3, 0.1); swingVine(-30, 17.3, -24.5, 9, 2.2, 1.2);
  swingVine(48, 16.3, -35.5, 8, 4.1, 0.4); swingVine(57, 25.8, 44, 12, 0.7, 2.0); swingVine(-58, 20.8, -20, 10, 2.8, 0.9); swingVine(-8, 10.8, 26, 6, 1.1, 1.8);

  // ---------------- the temple: three tiers, a tunnel through the base, stairs winding up ----------------
  const S = { ink: BL };
  box(0, 0, -7.75, 24, 5, 8.5, S); box(0, 0, 7.75, 24, 5, 8.5, S); box(0, 3.3, 0, 24, 1.7, 7, S);
  box(0, 5, 0, 16, 4.5, 16, S); box(0, 9.5, 0, 10, 4, 10, S); box(0, 13.5, 0, 4.4, 1.2, 4.4, S);
  stairs(0, 0, 19.2, '-z', 16, 3.2, { rise: 5 / 16, run: 0.45 }); stairs(0, 0, -19.2, '+z', 16, 3.2, { rise: 5 / 16, run: 0.45 });
  stairs(-9, 5, -11.6, '+z', 15, 1.9, { rise: 4.5 / 15, run: 0.45 }); stairs(6, 9.5, 7.6, '-z', 13, 1.9, { rise: 4 / 13, run: 0.45 });
  ring(0, 17.2, 0, 'y');
  // a carved face on the north tier, moss and creepers on the stone
  box(-2.4, 7.4, -8.06, 1.6, 0.6, 0.1, { noCollide: true, ink: BK }); box(2.4, 7.4, -8.06, 1.6, 0.6, 0.1, { noCollide: true, ink: BK }); box(0, 5.6, -8.06, 3.2, 0.5, 0.1, { noCollide: true, ink: BK }); box(0, 6.4, -8.06, 0.6, 1.2, 0.1, { noCollide: true, ink: BK });
  for (const [x, y, z, w, h, d] of [[-7, 1, 12.06, 5, 3.2, 0.1], [9, 0.5, 12.06, 4, 2.4, 0.1], [-12.06, 0.5, -4, 0.1, 3.5, 3], [12.06, 1.5, 6, 0.1, 3, 3], [8.06, 6, 3, 0.1, 3, 3], [-4, 10.5, -5.06, 3, 2.5, 0.1]]) box(x, y, z, w, h, d, { noCollide: true, ink: GR });
  for (const [x, z] of [[-12.2, -9], [12.2, 9], [9, -12.2], [-9, 12.2]]) box(x, 2, z, 0.14, 9, 0.14, { noCollide: true, ink: GR });
  // four pillars around the plaza, low walls for cover, a stone path from the south
  for (const [x, z] of [[-18, -18], [18, -18], [-18, 18], [18, 18]]) { cyl(x, 0, z, 1.1, 9, { seg: 8, ink: BL }); box(x, 9, z, 2.8, 0.5, 2.8, { ink: BL }); ring(x, 11, z, 'y'); }
  for (const [x, z, w, d] of [[-15, -6, 4, 0.7], [15, 6, 4, 0.7], [-6, -16, 0.7, 4], [6, 16, 0.7, 4]]) box(x, 0, z, w, 1.1, d, { ink: BL });
  for (let i = 0; i < 9; i++) box((i % 2 ? 0.9 : -0.9), 0, 22 + i * 2.4, 1.3, 0.12, 1.3, { noCollide: true, ink: BL });

  // ---------------- the cliff and the waterfall, with a cave behind the water ----------------
  box(52, 0, -66, 6, 15, 26, S); box(75.5, 0, -66, 7, 15, 26, S); box(63.5, 0, -70, 17, 15, 18, S); box(63.5, 3.6, -57, 17, 11.4, 8, S);
  box(70, 15, -70, 16, 4, 16, S); box(62, 19, -74, 5, 2, 5, S); ring(64, 22, -66, 'y'); ring(56, 16.5, -56, 'y');
  box(62, -0.4, -52.6, 5.5, 15.4, 0.5, { noCollide: true, ink: BL, fill: true });
  for (let k = 0; k < 6; k++) box(60 + k * 0.85, 2 + (k % 3) * 3, -52.2, 0.12, 5, 0.12, { noCollide: true, ink: BL });
  for (let k = 0; k < 7; k++) { const a = k * 0.9; sphere(62 + Math.cos(a) * 4, -0.1, -48 + Math.sin(a) * 2.5, 0.7 + (k % 2) * 0.3, { seg: 7, ink: BL }); }
  box(63.5, 0, -56, 3, 0.4, 3, { noCollide: true, ink: BK }); // a dry patch inside the cave
  for (const [x, z, h] of [[50, -52, 4], [78, -52, 5]]) box(x, 0, z, 2, h, 2, S);

  // ---------------- the expedition camp: a tent, a jeep, a fire, and everything that came in the crates ----------------
  { const g = new THREE.CylinderGeometry(2.4, 2.4, 5.5, 3); g.rotateZ(Math.PI / 2); g.rotateX(-Math.PI / 2); g.translate(-54, 1.2, 40); addGeo(g, OR); collider(-54, 0, 40, 5.5, 3.6, 4.2, {});
    box(-56.9, 0, 40, 0.16, 3.4, 0.16, { noCollide: true, ink: BK }); box(-51.1, 0, 40, 0.16, 3.4, 0.16, { noCollide: true, ink: BK }); }
  box(-40, 0.75, 62, 4.8, 1.4, 2.3, S); box(-41, 2.15, 62, 2.4, 1.1, 2.1, S); box(-38.2, 2.15, 62, 1.4, 0.15, 2.1, { noCollide: true, ink: BK });
  for (const [x, z] of [[-41.6, 63.3], [-38.4, 63.3], [-41.6, 60.7], [-38.4, 60.7]]) { const g = new THREE.CylinderGeometry(0.55, 0.55, 0.4, 10); g.rotateX(Math.PI / 2); g.translate(x, 0.55, z); addGeo(g, BK); }
  for (let k = 0; k < 7; k++) { const a = k * TAU / 7; sphere(-47 + Math.cos(a) * 1.3, 0.25, 48 + Math.sin(a) * 1.3, 0.3, { seg: 6, ink: BL }); }
  { const g = new THREE.CylinderGeometry(0.18, 0.18, 1.8, 6); g.rotateZ(Math.PI / 2); const g2 = g.clone(); g2.rotateY(Math.PI / 2); g.translate(-47, 0.2, 48); g2.translate(-47, 0.2, 48); addGeo(g, BK); addGeo(g2, BK);
    const flame = mesh(new THREE.ConeGeometry(0.5, 1.6, 6), OR); flame.position.set(-47, 0.9, 48); scene.add(flame); L.meshes.push(flame); L.animated.push({ mesh: flame, update: (t) => { flame.scale.set(1 + Math.sin(t * 9) * 0.12, 1 + Math.sin(t * 13) * 0.25, 1 + Math.cos(t * 11) * 0.12); flame.rotation.y = t * 2; } }); }
  for (const [x, z] of [[-44, 44], [-50, 46]]) { box(x, 0.4, z, 2.2, 0.14, 0.6, { ink: BK }); box(x, 0, z, 1.9, 0.4, 0.2, { noCollide: true, ink: BK }); }

  // ---------------- palms on the banks, fallen logs, boulders, stumps ----------------
  const palm = (x, z, h) => {
    for (let k = 0; k < 4; k++) { const g = new THREE.CylinderGeometry(0.28 - k * 0.03, 0.34 - k * 0.03, h / 4 + 0.2, 7); g.translate(x + k * 0.25, (k + 0.5) * h / 4, z + k * 0.12); addGeo(g, BK); }
    collider(x + 0.4, 0, z + 0.2, 0.9, h, 0.9, {});
    for (let k = 0; k < 7; k++) { const g = new THREE.BoxGeometry(0.6, 0.12, 4.2); g.translate(0, 0, 1.9); g.rotateX(0.55); g.rotateY(k * TAU / 7); g.translate(x + 1.0, h + 0.2, z + 0.5); addGeo(g, GR); }
    ring(x + 1, h + 1.8, z + 0.5, 'y');
  };
  palm(33, 18, 9); palm(52, 4, 11); palm(19, 46, 8); palm(3, 62, 10); palm(68, -38, 9);
  const log = (x, z, len, alongX, r = 0.6) => { const g = new THREE.CylinderGeometry(r, r * 1.08, len, 9); if (alongX) g.rotateZ(Math.PI / 2); else g.rotateX(Math.PI / 2); g.translate(x, r, z); addGeo(g, BK); collider(x, 0, z, alongX ? len : r * 2, r * 2, alongX ? r * 2 : len, {}); const e = new THREE.CylinderGeometry(r * 0.7, r * 0.7, 0.06, 9); if (alongX) { e.rotateZ(Math.PI / 2); e.translate(x + len / 2, r, z); } else { e.rotateX(Math.PI / 2); e.translate(x, r, z + len / 2); } addGeo(e, OR); };
  log(-20, -30, 9, true); log(36, -18, 8, false); log(-44, 26, 10, true); log(20, 44, 7, true); log(45, 1, 12, true, 0.55); log(-4, -60, 8, false); log(58, 62, 8, true);
  for (const [x, z, r] of [[8, -52, 2.0], [-58, -58, 2.4], [60, 64, 2.2], [-12, 62, 1.7], [42, -8, 1.5], [-66, 40, 2.0], [28, 8, 1.4], [-24, 6, 1.5]]) { sphere(x, r * 0.55, z, r, { seg: 8, ink: BL }); collider(x, 0, z, r * 1.5, r * 1.15, r * 1.5, {}); }
  for (const [x, z] of [[-34, 36], [26, -56], [-64, 62], [64, 24], [-40, -64], [8, 20]]) { cyl(x, 0, z, 0.8, 1.0, { seg: 8, ink: BK }); addGeo(new THREE.CylinderGeometry(0.62, 0.62, 0.06, 9).translate(x, 1.03, z), OR); }
  // giant mushrooms: platforms to hop between on the way up a trunk
  for (const [x, z, h, r] of [[-36, -20, 2.2, 1.6], [-40, -24, 3.6, 1.4], [24, -36, 2.4, 1.5], [-22, 50, 2.0, 1.7], [58, 40, 2.6, 1.4], [62, 36, 4.2, 1.2]]) { cyl(x, 0, z, 0.4, h, { seg: 7, ink: BL }); const g = new THREE.SphereGeometry(r, 10, 6, 0, TAU, 0, Math.PI / 2); g.scale(1, 0.55, 1); g.translate(x, h, z); addGeo(g, OR); collider(x, h, z, r * 1.7, r * 0.5, r * 1.7, {}); for (let k = 0; k < 3; k++) sphere(x + Math.cos(k * 2.1) * r * 0.55, h + r * 0.45, z + Math.sin(k * 2.1) * r * 0.55, 0.18, { seg: 5, ink: PK }); }

  // ---------------- undergrowth: ferns, flowers, little mushrooms (nothing you can bump into) ----------------
  let seed = 7; const srand = (a, b) => { seed = (seed * 16807) % 2147483647; return a + (seed / 2147483647) * (b - a); };
  const open = (x, z) => Math.abs(x) > 15 || Math.abs(z) > 15;
  for (let i = 0; i < 46; i++) { const x = srand(-74, 74), z = srand(-74, 74); if (!open(x, z) || Math.abs(x - riverX(z)) < riverW(z) / 2 + 1) continue; for (let k = 0; k < 5; k++) { const g = new THREE.BoxGeometry(0.16, 1.1 + srand(0, 0.8), 0.05); g.translate(0, 0.6, 0); g.rotateZ(srand(-0.7, 0.7)); g.rotateY(k * 1.26); g.translate(x, 0, z); addGeo(g, GR); } }
  for (let i = 0; i < 30; i++) { const x = srand(-74, 74), z = srand(-74, 74); if (!open(x, z) || Math.abs(x - riverX(z)) < riverW(z) / 2 + 1) continue; box(x, 0, z, 0.07, 0.9, 0.07, { noCollide: true, ink: GR }); sphere(x, 1.0, z, 0.24, { seg: 6, ink: i % 4 ? PK : OR }); }
  for (let i = 0; i < 18; i++) { const x = srand(-74, 74), z = srand(-74, 74); if (!open(x, z) || Math.abs(x - riverX(z)) < riverW(z) / 2 + 1) continue; cyl(x, 0, z, 0.12, 0.45, { noCollide: true, seg: 5, ink: BL }); const g = new THREE.SphereGeometry(0.34, 7, 4, 0, TAU, 0, Math.PI / 2); g.scale(1, 0.6, 1); g.translate(x, 0.42, z); addGeo(g, OR); }

  // ---------------- loose props: they tumble, bounce and settle where they land ----------------
  const crate = (x, y, z, yaw = 0) => prop('crate', x, y, z, { x: 0.6, y: 0.6, z: 0.6 }, (g) => { g.add(mesh(new THREE.BoxGeometry(1.2, 1.2, 1.2), BL)); for (const k of [-1, 1]) { g.add(mesh(new THREE.BoxGeometry(1.24, 0.14, 1.24).translate(0, k * 0.36, 0), OR)); g.add(mesh(new THREE.BoxGeometry(1.24, 1.24, 0.14).translate(0, 0, k * 0.36), OR)); } g.add(mesh(new THREE.BoxGeometry(0.5, 0.36, 0.02).translate(0, 0.02, 0.62), BK)); }, { mass: 1, snap: 'cube', ink: BL, yaw });
  const barrel = (x, y, z) => prop('barrel', x, y, z, { x: 0.52, y: 0.62, z: 0.52 }, (g) => { g.add(mesh(new THREE.CylinderGeometry(0.5, 0.5, 1.24, 10), OR)); for (const yy of [-0.36, 0.36]) g.add(mesh(new THREE.TorusGeometry(0.5, 0.05, 4, 14).rotateX(Math.PI / 2).translate(0, yy, 0), BK)); g.add(mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.08, 8).translate(0.2, 0.64, 0.1), BK)); }, { mass: 1.3, snap: 'cube', ink: OR });
  const boulder = (x, y, z, r = 0.85) => prop('boulder', x, y, z, { x: r, y: r, z: r }, (g) => { g.add(mesh(new THREE.DodecahedronGeometry(r, 1), BL)); }, { mass: 3.2, snap: 'free', ink: BL, radius: r, footHalf: r * 0.85 });
  const coconut = (x, y, z) => prop('coconut', x, y, z, { x: 0.3, y: 0.3, z: 0.3 }, (g) => { g.add(mesh(new THREE.SphereGeometry(0.3, 8, 6), BK)); for (let k = 0; k < 3; k++) g.add(mesh(new THREE.SphereGeometry(0.05, 4, 3).translate(Math.cos(k * 2.1) * 0.12, 0.27, Math.sin(k * 2.1) * 0.12), OR)); }, { mass: 0.25, snap: 'free', ink: BK, radius: 0.3 });
  const chunk = (x, y, z, yaw = 0) => prop('log', x, y, z, { x: 0.9, y: 0.45, z: 0.45 }, (g) => { g.add(mesh(new THREE.CylinderGeometry(0.45, 0.45, 1.8, 9).rotateZ(Math.PI / 2), BK)); for (const k of [-1, 1]) g.add(mesh(new THREE.CylinderGeometry(0.34, 0.34, 0.05, 9).rotateZ(Math.PI / 2).translate(k * 0.9, 0, 0), OR)); }, { mass: 1.6, snap: 'yaw', ink: BK, footHalf: 0.45, yaw });
  const idol = (x, y, z, yaw = 0) => prop('idol', x, y, z, { x: 0.6, y: 0.8, z: 0.55 }, (g) => { g.add(mesh(new THREE.BoxGeometry(1.2, 1.6, 1.1), BL)); g.add(mesh(new THREE.BoxGeometry(0.9, 0.3, 0.1).translate(0, 0.35, 0.56), BK)); g.add(mesh(new THREE.BoxGeometry(0.5, 0.35, 0.1).translate(0, -0.35, 0.56), BK)); g.add(mesh(new THREE.BoxGeometry(0.25, 0.5, 0.2).translate(0, 0.05, 0.6), BL)); g.add(mesh(new THREE.BoxGeometry(1.4, 0.3, 1.3).translate(0, 0.85, 0), OR)); }, { mass: 4, snap: 'cube', ink: BL, yaw });
  crate(-49, 0, 50, 0.3); crate(-47.6, 0, 52.2); crate(-52, 0, 44.5, -0.5); crate(-51, 1.2, 44.5); crate(-16, 0, 14); crate(13, 0, -16, 0.4); crate(64, 0, -58); crate(52, 0, 12, 0.8); crate(-58, 0, -40); crate(20, 0, 60);
  barrel(-44, 0, 40); barrel(-45.2, 0, 41.3); barrel(-37, 0, 60); barrel(60, 0, -57); barrel(-8, 0, 12);
  boulder(20, 0, -48); boulder(-36, 0, 30); boulder(44, 0, 30, 0.7); boulder(-6, 0, -22); boulder(12, 0, 58, 0.75); boulder(-60, 0, 20, 0.95);
  coconut(34.5, 0, 16); coconut(35.6, 0, 19.2); coconut(50.4, 0, 6); coconut(53, 0, 2.2); coconut(17.5, 0, 44.6); coconut(20.6, 0, 47.4); coconut(1.6, 0, 60); coconut(66.5, 0, -36);
  chunk(-20, 0, -27.4, 0); chunk(-22.6, 0, 32, 0); chunk(38, 0, -22.4, 1.5708); chunk(-2, 0, -58, 1.5708);
  idol(0, 14.7, 0, 3.1416); idol(-12.5, 0, -14, 0.8); idol(14, 0, 13, -2.2);

  // ---------------- birds overhead, a fat sun, the jungle beyond the walls ----------------
  birds(4, 34, 30, { rStep: 9, hStep: 5, speed: 0.1, scale: 1.4 });
  addGeo(new THREE.SphereGeometry(13, 14, 10).translate(-110, 118, -170), OR);
  for (let i = 0; i < 12; i++) { const a = (i / 12) * TAU; const g = new THREE.BoxGeometry(6.5, 0.9, 0.9); g.rotateZ(a); g.translate(-110 + Math.cos(a) * 20, 118 + Math.sin(a) * 20, -170); addGeo(g, OR); }
  for (let i = 0; i < 22; i++) { const a = i * (TAU / 22) + 0.2, r = 104 + (i % 4) * 12, x = Math.cos(a) * r, z = Math.sin(a) * r, h = 28 + (i * 7) % 16, cr = 9 + (i % 3) * 3; addGeo(new THREE.CylinderGeometry(1.4, 2.2, h, 7).translate(x, h / 2, z), BK); for (let k = 0; k < 4; k++) { const g = new THREE.SphereGeometry(cr * (k ? 0.6 : 0.9), 8, 6); g.scale(1, 0.55, 1); g.translate(x + (k ? Math.cos(k * 2.1) * cr * 0.6 : 0), h + (k % 2), z + (k ? Math.sin(k * 2.1) * cr * 0.6 : 0)); addGeo(g, GR); } }
  for (const [x, z, r, h] of [[150, -190, 60, 55], [-190, -120, 50, 42], [200, 80, 70, 60], [-60, 220, 80, 50]]) addGeo(new THREE.ConeGeometry(r, h, 6).translate(x, h / 2, z), BL);
  for (const [cx, cy, cz, s] of [[70, 80, -180, 1.1], [-40, 88, -200, 1.3], [160, 70, -60, 0.9], [-170, 74, 50, 1.1], [40, 84, 190, 1.2]]) for (let i = 0; i < 6; i++) sphere(cx + (i - 2.5) * 5 * s, cy + Math.sin(i * 1.7) * 2.5 * s, cz, (4 + (i % 3)) * s, { seg: 9 });

  // ---------------- where things are ----------------
  for (const [x, y, z] of [[-46, 18.2, -46], [18, 22.2, -30], [-28, 20.2, 54], [48, 25.6, -44], [0, 13.5, 3.5], [64, 19, -66], [-32, 28.6, -14], [62, 26.2, 44]]) sniper(x, y, z);
  for (const [x, y, z] of [[-70, 0, 0], [70, 0, 30], [0, 0, -70], [-60, 0, 66], [40, 0, -68], [-70, 0, -66], [70, 0, 68], [-6, 0, 70], [24, 0, -6], [-28, 0, 24], [-66, 0, -34], [56, 0, 26]]) spawn(x, y, z);
  for (const [x, y, z] of [[0, 5, 10], [0, 9.5, -7], [-18, 0, 0], [18, 0, 0], [0, 0, -24], [-46, 18.2, -46], [18, 22.2, 0], [-28, 20.2, 54], [63.5, 0, -57], [-50, 0, 47], [-32, 28.6, -12], [48, 0, 24], [-56, 15.2, 12], [10, 0, 40], [-40, 0, -46], [30, 0, -46]]) pickup(x, y, z);
  for (const [x, y, z] of [[-70, 0, 0], [70, 0, 30], [0, 0, -70], [-60, 0, 62], [40, 0, -66], [-46, 18.2, -46], [18, 22.2, -30], [-28, 20.2, 54], [48, 25.6, -44], [62, 26.2, 44], [-56, 15.2, 12], [6.5, 9.5, -6.5], [64, 19, -66], [63.5, 0, -57], [-52, 0, 44], [56, 16.2, 8], [-8, 0, 70], [22, 0, 6]]) L.arenaSpawns.push(new THREE.Vector3(x, y, z));
  B.finish(); return L;
}

export function buildLevel(scene, world, key = 'district', opts = {}) {
  const B = createBuilder(scene, world);
  return key === 'mexico' ? buildMexico(B, !!opts.arena) : key === 'jungle' ? buildJungle(B, !!opts.arena) : buildDistrict(B, !!opts.arena);
}
