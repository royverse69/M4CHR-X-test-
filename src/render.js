...
// the district's own colours, kept so a map with its own palette can hand them back
const DEFAULT_INKS = INK_COLORS.map((c) => c.toArray());
export const DEFAULT_STYLE = { paper: [0.965, 0.955, 0.905], lines: 0, inks: DEFAULT_INKS };
...
void main() {
  ...
  if (uLineMode < 0.5) {
    ...
    // graph paper: a light green grid, the field-notebook look
    float gs = ls * 0.5;
    float gx = mod(pp.x + gs * 0.5, gs), gy = mod(pp.y + gs * 0.5, gs);
    float g1 = 1.0 - smoothstep(0.4 * sc, 1.3 * sc, abs(gx - gs * 0.5));
    float g2 = 1.0 - smoothstep(0.4 * sc, 1.3 * sc, abs(gy - gs * 0.5));
    paper = mix(paper, vec3(0.55, 0.74, 0.60), max(g1, g2) * 0.34);
  }
...
export class InkRenderer {
  constructor(canvas) {
    ...
    this.post = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uLowHp: { value: 0 }, uLineSpacing: { value: 60 }, uLineMode: { value: 0 }, uPaper: { value: new THREE.Vector3(0.965, 0.955, 0.905) }, uInks: { value: INK_COLORS },
  ...
  // a map can bring its own paper tint, ruling and ink palette; nothing given means the district look
  setStyle(style) {
    const s = { ...DEFAULT_STYLE, ...(style || {}) }; const u = this.post.uniforms;
    u.uPaper.value.fromArray(s.paper); u.uLineMode.value = s.lines || 0;
    for (let i = 0; i < INK_COLORS.length; i++) INK_COLORS[i].fromArray((s.inks && s.inks[i]) || DEFAULT_INKS[i]);
  }
  resize() {
