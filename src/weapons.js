...
export class Gun extends ViewModel {
  ...
  fireRay(origin, dir) {
    ...
    } else if (hitW && hitW.box && hitW.box.data.prop && (!hitE || hitW.dist < hitE.dist) && ctx.propHit) {
      end = hitW.point; ctx.propHit(hitW.box.data.prop, this.damage, hitW.point, dir); ctx.effects.bulletImpact(hitW.point, hitW.normal, INK.BLUE); hit = true;
...
export class Katana extends ViewModel {
  ...
  doHit(st, s) {
    ...
    if (ctx.propsInArc) for (const pr of ctx.propsInArc(P.eye, P.forward, 3.2, Math.cos(1.0))) { any = true; ctx.batProp(pr, _v2.clone()); }
