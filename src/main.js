...
import { buildLevel, LEVELS, DEV_LEVELS, ALL_LEVELS } from './level.js';
...
// test features (maps still being worked on) open up with a code typed on the main menu
const DEV_CODE = 'evdevops';
let devMode = localStorage.getItem('doodle_dev') === '1';
const visibleLevels = () => (devMode ? ALL_LEVELS : LEVELS);
// any map the game can build is a valid choice from a host, whether or not this player has unlocked it
const knownMap = (k) => (ALL_LEVELS.some((m) => m.key === k) ? k : 'district');
let mapKey = knownMap(localStorage.getItem('doodle_map') || 'district');
if (!visibleLevels().some((m) => m.key === mapKey)) mapKey = 'district';
...
audio.setTune(mapKey === 'mexico' ? 'mexico' : 'district'); R.setStyle(level.style);
...
function setLevel(key, on, force = false) {
  ...
  level = buildLevel(R.scene, world, key, { arena: on }); nav = new NavGrid(world, level.bounds, 1).build(); props.attach(level);
  ctx.level = level; ctx.nav = nav; if (window.__game) { window.__game.level = level; window.__game.nav = nav; }
  audio.setTune(key === 'mexico' ? 'mexico' : 'district'); R.setStyle(level.style);
}
...
const props = new Props(ctx); props.attach(level);
...
const lobby = { players: new Map(), hostId: null, isPublic: true, status: '', code: '', map: null, gen: 0, next: null, token: null };
const scores = new Map();      // peer id -> { name, kills, deaths }
let screen = 'main';           // which start-screen panel is showing: main | online | lobby
window.__game = { ctx, game, player, enemies, nav, world, level, hud, effects, input, net, remote, lobby, scores, props, setLevel };
...
ctx.hitPlayer = (t, dmg, info) => {
  ...
  effects.blood(info.point, info.dir, clamp(0.4 + dmg / 80, 0.4, 1.6), { ink: INK.RED }); hud.hitmarker(predictKill(t, dmg), info.crit); audio.hitEnemy(t.center); t.flash();
...
// The victim decides its own death, and hearing about it takes a round trip. When our own sums say
// a hit was lethal (their last snapshot's health, minus what we have done since, no spawn shield),
// the kill marker and sound play right away; the feed line and the score follow with the confirmation.
const predicted = new Map();
function predictKill(t, dmg) {
  const fresh = t.lastSeen && performance.now() - t.lastSeen < 900;
  if (!fresh || t.shielded || t.hp <= 0) return false;
  if (dmg < t.hp - 0.5) { t.hp -= dmg; return false; }
  t.hp = 0; predicted.set(t.id, performance.now()); audio.kill(true); return true;
}
...
// loose props get shoved instead of broken; the one who shoved tells the others
ctx.propHit = (p, dmg, point, dir) => props.hit(p, dmg, point, dir);
ctx.blastProps = (c, R) => props.blast(c, R);
ctx.propsInArc = (pos, dir, range, cosHalf) => props.inArc(pos, dir, range, cosHalf);
ctx.batProp = (p, dir) => props.bat(p, dir);
props.onKick = (p) => { if (net.active && inMatch()) net.broadcast('pk', props.pack(p)); };
props.onRest = (p) => { if (net.active && inMatch()) net.broadcast('pr', props.pack(p)); };
...
function makePickup(kind) {
  ...
  else if (level.key === 'jungle') { const b = new THREE.Mesh(new THREE.TorusGeometry(0.42, 0.11, 6, 10, 2.4), pmat.shell); b.rotation.z = -1.9; g.add(b); const tip = new THREE.Mesh(new THREE.SphereGeometry(0.09, 5, 4), pmat.cap); tip.position.set(0.1, -0.4, 0); g.add(tip); }
...
function collectPickup(p) {
  if (p.kind === 'ammo') { player.addAmmoAll(0.4); player.grenades = Math.min(player.maxGrenades, player.grenades + 1); hud.kill('+AMMO · +GRENADE', 0); } else { player.hp = Math.min(player.maxHp, player.hp + 35); hud.kill(level.key === 'mexico' ? 'TACO · +35 HP' : level.key === 'jungle' ? 'BANANA · +35 HP' : '+35 HP', 0); }
...
function addRemote(id, name) {
  ...
  rp.onDamage = (t, amount, fromPos) => { if (!ctx.canHurt(t) || !t.alive) return; hud.hitmarker(predictKill(t, amount), false); net.sendTo(t.id, 'pdmg', { amount: Math.round(amount), from: fromPos ? fromPos.toArray().map((v) => +v.toFixed(1)) : null, by: net.id, src: 'grenade' }); };
  remote.set(id, rp); return rp;
}
function removeRemote(id) { const r = remote.get(id); if (r) { r.dispose(); remote.delete(id); } lobby.players.delete(id); scores.delete(id); peerStats.delete(id); net.unlink(id); }
function lobbyRows() { return [...lobby.players.entries()].map(([id, p]) => ({ id, name: p.name, v: p.v || 1 })); }
function broadcastLobby() { lobby.next = pickSuccessor(); net.send('lobby', { players: lobbyRows(), hostId: net.id, isPublic: lobby.isPublic, map: lobby.map || mapKey, shown: net.aliasCode || net.code, next: lobby.next, gen: lobby.gen || 0 }); renderLobby(); }
// who takes over if this host vanishes: the player best connected to everyone else (most direct
// links open, lowest ping), the earliest joiner on a tie. Everyone hears the choice with the roster.
const peerStats = new Map();
function pickSuccessor() {
  if (!net.isHost) return lobby.next || null;
  const ids = [...lobby.players.keys()].filter((id) => id !== net.id); if (!ids.length) return null;
  const order = lobby.order || ids; const rank = (id) => { const i = order.indexOf(id); return i < 0 ? 999 : i; };
  const key = (id) => { const st = peerStats.get(id) || {}; return { links: st.links ?? -1, rtt: st.rtt ?? 9 }; };
  ids.sort((a, b) => { const A = key(a), B = key(b); return (B.links - A.links) || (A.rtt - B.rtt) || (rank(a) - rank(b)); });
  return ids[0];
}
net.on('stat', (d, from) => { if (net.isHost && d) peerStats.set(from, { links: Number(d.links) || 0, rtt: typeof d.rtt === 'number' ? d.rtt : null }); });
...
async function _migrateHost() {
  const oldHost = lobby.hostId, myId = net.id; const gen = (lobby.gen || 0) + 1; const token = lobby.token || net.token || null;
  const base = (lobby.code || net.code || '').replace(/-\d+$/, '');
  const alive = (lobby.order || []).filter((id) => id !== oldHost && lobby.players.has(id));
  // the host's own pick goes first, then everyone in joining order; each waits their turn to take over,
  // so if the first choice has gone too the next one steps up instead of everybody giving up
  const cands = [lobby.next, ...alive].filter((id, i, a) => id && id !== oldHost && lobby.players.has(id) && a.indexOf(id) === i);
  const myRank = cands.indexOf(myId); if (!cands.length || !base || myRank < 0) { leaveOnline('the host left the lobby'); return; }
  ...
  const started = performance.now(); let msgT = 0;
  const say = () => { if (performance.now() - msgT < 2500) return; msgT = performance.now(); hud.message('HOST LEFT', myRank === 0 ? 'you are hosting now · hold on' : 'connecting to the new host now · this only takes a few seconds', 3); };
  const deadline = started + 45000; let hosting = false, joined = false, hostTried = false;
  while (!hosting && !joined && performance.now() < deadline) {
    say();
    if (!hostTried && performance.now() - started >= myRank * 7000) {
      hostTried = true;
      try { const r = await net.hostGen(base, gen, token, lobby.isPublic); lobby.gen = r.gen; hosting = true; break; } catch (e) { /* someone else took over, or signalling is down: keep looking for them */ }
    }
    try { await net.join(base, { name: myName, prev: myId }, { token, gens: [gen, gen + 1, gen + 2, gen + 3, gen + 4, gen + 5], timeout: 6000 }); joined = true; }
    catch (e) { await sleep(900); }
  }
  if (hosting) {
    const mine = lobby.players.get(myId) || { name: myName }; mine.v = PROTO; lobby.players.delete(myId); lobby.players.set(net.id, mine);
    const ms = scores.get(myId); scores.delete(myId); if (ms) scores.set(net.id, ms);
    lobby.hostId = net.id; lobby.code = net.code; lobby.token = net.token; lobby.order = [net.id, ...alive.filter((id) => id !== myId)]; lobby.next = null; peerStats.clear(); net.accepting = true; game.clockStarted = clockRunning || matchLeft < FFA_TIME;
    net.onAlias = () => { broadcastLobby(); hud.kill('lobby code ' + base + ' is back', 0); }; net.claimAlias(base);
    hud.message('YOU ARE THE HOST NOW', 'the others are on their way back', 2.5);
  ...
  lobby.code = net.code; lobby.token = net.token; hud.message('BACK IN', 'connected to the new host', 1.6); if (!wasInMatch) { game.state = 'lobby'; screen = 'lobby'; showStart(); }
}
...
net.onPeerJoin = (from, meta) => {
  ...
  lobby.players.set(from, { name, v: meta && meta.v || 1 }); addRemote(from, name); broadcastLobby();
  if (game.state === 'play' || game.state === 'dying') { if (!scores.has(from)) scores.set(from, { name, kills: 0, deaths: 0 }); net.sendTo(from, 'start', { late: true, spawn: farthestSpawnIndex(), map: lobby.map || mapKey, broken: level.breakables.filter((b) => !b.alive).map((b) => b.id), props: props.state() }); sendScores(); hud.kill(name + ' joined', 0); }
};
net.on('lobby', (d) => {
  ...
  lobby.next = d.next || null; if (d.gen != null) lobby.gen = d.gen; lobby.token = net.token;
  for (const p of d.players) lobby.players.set(p.id, { name: p.name, v: p.v || 1 });
  // direct links to everyone on the new game; the old game's players are reached through the host
  net.setRoster(lobby.order, d.players.filter((p) => !(p.v >= 2)).map((p) => p.id));
...
net.on('start', (d) => { if (net.isHost) return; if (d.map) lobby.map = knownMap(d.map); startMatch(!!d.late, d.spawns ? d.spawns[net.id] : d.spawn); if (d.broken) for (const id of d.broken) { const br = level.breakables[id]; if (br) breakProp(br, null, false, true); } if (d.props) props.applyState(d.props); });
net.on('pk', (d) => props.applyKick(d)); net.on('pr', (d) => props.applyRest(d));
...
net.on('pdead', (d, from) => {
  ...
  if (d.killer === net.id) { game.kills++; game.addScore(100, 'ERASED ' + vn + how); const pt = predicted.get(from); predicted.delete(from); if (!pt || performance.now() - pt > 2500) audio.kill(true); }
...
let psT = 0, statT = 0, lobbyT = 0;
function netUpdate(dt) {
  idleUpdate(dt);
  if (!net.active) return; const now = performance.now() / 1000;
  ...
  // twenty snapshots a second whatever the frame rate, on the lossy lane; tracers ride along
  psT += dt; if (psT >= 0.05 && inMatch()) { psT = 0; net.sendFast('ps', encodeLocal(player, player.weaponIndex, { firing: player.firing, idle: input.idleSeconds > IDLE_FLAG, shield: player.shieldT > 0 })); }
  if (shotQueue.length) net.sendFast('shots', { k: player.weapon.kind, e: shotQueue.splice(0) });
  // every few seconds a client tells the host how well connected it is, and the host repeats the
  // roster (with its stand-in choice) so everyone agrees on who takes over if it goes
  statT += dt; if (statT >= 5) { statT = 0; if (!net.isHost) net.meanRtt().then((rtt) => { if (net.active && !net.isHost) net.send('stat', { rtt, links: net.directCount }); }); }
  if (net.isHost && inMatch()) { lobbyT += dt; if (lobbyT >= 10) { lobbyT = 0; broadcastLobby(); } }
...
async function createLobby(isPublic) {
  ...
  lobby.isPublic = isPublic; lobby.map = mapKey; lobby.players.clear(); lobby.players.set(net.id, { name: myName, v: PROTO }); lobby.hostId = net.id; lobby.status = ''; lobby.token = net.token; lobby.gen = 0; lobby.next = null; peerStats.clear();
...
const mapName = (k) => (ALL_LEVELS.find((m) => m.key === k) || LEVELS[0]).name;
function mapHTML(sel, canPick) {
  const list = [...visibleLevels()]; if (!list.some((m) => m.key === sel)) { const m = ALL_LEVELS.find((x) => x.key === sel); if (m) list.push(m); }
  if (list.length < 2) return '';
  return `<div class="mapsel" id="mapsel"><span>map</span>${list.map((m) => `<button type="button" class="mapbtn${m.key === sel ? ' on' : ''}" data-map="${m.key}" ${canPick ? '' : 'disabled'}>${m.name}<i>${m.blurb}</i></button>`).join('')}</div>`;
}
// the code box on the main menu: the right word switches the test features on
function codeHTML() {
  if (devMode) return `<div class="codebox" id="codebox"><span>test features on · ${DEV_LEVELS.map((m) => m.name.toLowerCase()).join(' + ')} unlocked</span><button type="button" id="devOff">turn off</button></div>`;
  return `<div class="codebox" id="codebox"><input type="text" id="devCode" placeholder="enter code here" maxlength="24" autocomplete="off" spellcheck="false"></div>`;
}
// the box lives in the corner of the screen, outside the panel, so it stays out of the way
function wireCode() {
  const old = hud.el.screen.querySelector('#codebox'); if (old) old.remove();
  if (screen !== 'main') return;
  hud.el.screen.insertAdjacentHTML('beforeend', codeHTML()); const box = hud.el.screen.querySelector('#codebox'); if (!box) return;
  box.addEventListener('click', (e) => e.stopPropagation()); box.addEventListener('keydown', (e) => e.stopPropagation());
  const inp = box.querySelector('#devCode'), off = box.querySelector('#devOff');
  if (inp) inp.addEventListener('input', () => { const v = inp.value.trim().toLowerCase(); if (v === DEV_CODE) { devMode = true; localStorage.setItem('doodle_dev', '1'); showStart(); } else if (v.length >= DEV_CODE.length) inp.classList.add('nope'); else inp.classList.remove('nope'); });
  if (off) off.addEventListener('click', () => { devMode = false; localStorage.setItem('doodle_dev', '0'); if (!visibleLevels().some((m) => m.key === mapKey)) { mapKey = 'district'; localStorage.setItem('doodle_map', mapKey); } showStart(); });
}
...
function lobbyListHTML() {
  ...
  return lobbyList.map((l) => `<div class="lobbyrow"><span class="code">${esc(l.code)}</span><span>${esc(l.hostName || 'someone')}'s lobby</span><span>${l.players}/${l.max} players${l.inMatch ? ' · in a match' : ''}</span>${l.full ? '<span class="full">FULL</span>' : `<button type="button" data-join="${esc(l.code)}">JOIN</button>`}</div>`).join('');
}
async function refreshLobbies(again = false) {
  if (listBusy || net.active) return; listBusy = true; const box = hud.el.panel.querySelector('#lobbyRows'); if (box && !again) box.innerHTML = lobbyListHTML();
  let err = null, found = null; try { found = await net.listLobbies({ name: myName }); } catch (e) { found = []; err = e; }
  listBusy = false; if (again && !found.length) return; lobbyList = found;
  const rows = hud.el.panel.querySelector('#lobbyRows'); if (rows) rows.innerHTML = err ? `<div class="hint">could not look: ${esc(friendlyError(err))}</div>` : lobbyListHTML();
  // a probe can miss a lobby that is there (the signalling server is not always quick); an empty first look gets a quiet second one
  if (!again && !err && !found.length) setTimeout(() => { if (screen === 'online' && !net.active && hud.el.panel.querySelector('#lobbyRows')) refreshLobbies(true); }, 2500);
}
function wireOnline() {
...
// ---------------- run control ----------------
function resetGame() {
  if (level.breakables.some((b) => !b.alive)) setLevel(loadedKey, arenaLoaded, true); else props.reset();
...
function beginCommon() { audio.init(); audio.resume(); if (!input.usingGamepad && !input.isTouch) input.requestLock(); if (musicWanted && !audio.musicPlaying) audio.musicOn(true); hud.hideScreen(); hud.setGameplayVisible(true); game.menu = false; }
...
function startMatch(late, spawnIdx) {
  ...
  setTimeout(() => { if (game.state === 'play' && !input.pointerLocked && !input.usingGamepad && !input.isTouch) { game.menu = true; showClickToPlay(); } }, 250);
}
...
function resume() { if (online()) { game.menu = false; if (game.state === 'dying' && game.respawnT <= 0) game.respawnArm = input.lastActive; hud.hideScreen(); hud.setGameplayVisible(true); if (!input.usingGamepad && !input.isTouch) input.requestLock(); return; } begin(); }
...
input.onLockChange = (locked) => { if (!locked && (game.state === 'play' || (game.state === 'dying' && online())) && !game.menu && !input.usingGamepad && !input.isTouch) pause(); };
...
function step(now) {
  ...
  if (st === 'play' && !game.menu && !input.pointerLocked && !input.usingGamepad && !input.isTouch) { lockTipT -= dt; if (lockTipT <= 0) { lockTipT = 2.5; hud.tip('click the page to grab the mouse', 2); } }
  let scale = 1;
  ...
  if (playing) {
    ...
    player.update(sdt); enemies.update(sdt); props.update(sdt); effects.update(sdt); updatePickups(sdt); netUpdate(dt);
